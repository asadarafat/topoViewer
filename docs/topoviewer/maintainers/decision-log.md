...
| ADR-0004 | Accepted | Grafana user workflow is mounted `*.topo.tv.yaml`, `*.style.tv.yaml`, and `*.mapper.tv.yaml` bundles, not fixture catalog edits. | Grafana integration. | [Grafana TopoViewer Containerlab Lab](../labs/grafana-topoviewer-containerlab-lab.md) |
...
| ADR-0008 | Accepted | Runtime telemetry overlays must not mutate topology or stylesheet source YAML. | Grafana mapper and operational state. | [Grafana TopoViewer Containerlab Lab](../labs/grafana-topoviewer-containerlab-lab.md), [Browser harness](../tools/browser-harness.md) |
