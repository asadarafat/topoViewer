...
const topologyCollections: Partial<Record<StyleTargetKind, ['graph' | 'diagram', string]>> = {
  callout: ['diagram', 'callouts'],
  link: ['graph', 'links'],
  node: ['graph', 'nodes'],
  path: ['graph', 'paths'],
  region: ['graph', 'regions'],
  shape: ['diagram', 'shapes'],
  text: ['diagram', 'texts']
};

function targetPath(source: ParsedStudioSource, target: StudioStylesheetTarget): StudioYamlPath | undefined {
  if (target.kind === 'linkDirection') {
    const links = valueAt(source.value, ['graph', 'links']);
    if (!Array.isArray(links)) return undefined;
    for (const [linkIndex, linkValue] of links.entries()) {
  ...
  const collection = topologyCollections[target.kind];
  if (!collection) return undefined;
  const values = valueAt(source.value, collection);
  if (!Array.isArray(values)) return undefined;
  const index = values.findIndex((value) => record(value)?.id === target.id);
...
export function inlineStyleWinner(
  ...
  if (!objectPath) return { exists: false };
  const path = [...objectPath, 'style', ...fieldPath];
  const value = valueAt(parsed.source.value, path);
  return value === undefined ? { exists: false, path } : { exists: true, path, value };
}

export function migrateInlineStylesToCandidate({
  fieldPaths,
  stylesheetText,
  target,
  topologyText
  ...
  const parsedTopology = parseStudioSource('topology', topologyText);
  if (!parsedTopology.ok) return { diagnostics: parsedTopology.diagnostics, status: 'invalid' };
  ...
  const values = fieldPaths.flatMap((fieldPath) => {
    const value = valueAt(parsedTopology.source.value, [...objectPath, 'style', ...fieldPath]);
  ...
  for (const { fieldPath, value } of values) {
    const mutation = setCandidateStyleField(nextStylesheet, target, fieldPath, value);
    if (mutation.status === 'invalid') return mutation;
    if (mutation.status === 'normalization-required') {
      return {
        reason: mutation.reason,
        status: 'normalization-required',
        stylesheetText: mutation.after,
        topologyText
      };
    }
    nextStylesheet = mutation.text;
    updatedSelectors.push(...mutation.updatedSelectors);
  }

  let nextTopology = topologyText;
  for (const { fieldPath } of values) {
    ...
    nextTopology = removed;
  }

  const cleaned = parseStudioSource('topology', nextTopology);
  if (!cleaned.ok) return { diagnostics: cleaned.diagnostics, status: 'invalid' };
  const inlineStyleValue = valueAt(cleaned.source.value, [...objectPath, 'style']);
