...
import { migrateInlineStylesToCandidate, type StudioStylesheetTarget, type StudioYamlPath } from '../session';

interface StyleRuleSelectionOptions {
  selection: StudioSelection[];
}

function styleRuleCommand(id: string, label: string, mutations: StudioSourceMutation[], selection: StudioSelection[]): StudioCommand {
  return {
    id,
    label,
...
export function createStudioInlineStyleMigrationCommand({ fieldPaths, selection, target }: { fieldPaths: StudioYamlPath[]; selection: StudioSelection[]; target: StudioStylesheetTarget }): StudioCommand {
  return {
...
export function createStudioStyleRuleCommand({
  ...
  return styleRuleCommand('create-style-rule', `Create style rule ${selector}`, mutations, selection);
}
...
export function duplicateStudioStyleRuleCommand({ rule, selection }: StyleRuleSelectionOptions & { rule: StyleRule }): StudioCommand {
  return styleRuleCommand(
...
export function deleteStudioStyleRuleCommand({ index, selection }: StyleRuleSelectionOptions & { index: number }): StudioCommand {
  return styleRuleCommand(
...
export function createStudioStyleActions({ execute, session }: { execute(command: StudioCommand): boolean; session: StudioDocumentSession }) {
  function commitStyleInspector(request: StudioStyleEditRequest) {
    ...
    const scopePath = request.scope.kind === 'object' ? request.objectPath! : ['stylesheet', request.scope.ruleIndex];
    const path = request.scope.kind === 'object' ? [...request.objectPath!, 'style', ...request.fieldPath] : ['stylesheet', request.scope.ruleIndex, 'style', ...request.fieldPath];
    ...
    return execute({
      ...
      ...styleRuleCommand(`style-${document}-${path.join('-')}`, `Edit ${request.fieldPath.at(-1)}`, [mutation], selection ? [selection] : [])
    });
  }
  ...
  function unsetStyleInspector(request: StudioStyleUnsetRequest) {
    ...
    return execute(styleRuleCommand(`unset-style-${document}-${path.join('-')}`, `Unset ${request.fieldPath.at(-1)}`, [{ document, kind: 'remove-value', path, scopePath }], selection ? [selection] : []));
  }
  ...
  function deleteStyleRule(index: number) {
    const current = session.snapshot();
    return current.projection.document.stylesheet?.[index] ? execute(deleteStudioStyleRuleCommand({ index, selection: current.selection })) : false;
  }
