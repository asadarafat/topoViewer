import { mapperAuthoringMetadata, type MapperAuthoringFieldMetadata } from 'topoviewer/authoring';
...
export function mapperFieldDisposition(field: MapperAuthoringFieldMetadata): StudioMapperFieldDisposition {
  return field.path.includes('.*') || (field.path.match(/\[\]/g) || []).length > 1 || field.valueType === 'object' || field.valueType === 'array' || field.control?.kind === 'nested' ? 'raw-yaml' : 'control';
}

export function mapperFieldIsDirectlyEditable(field: MapperAuthoringFieldMetadata, rule?: StudioMapperRuleReference): boolean {
...
export function mapperFieldSourcePath(field: MapperAuthoringFieldMetadata, rule?: StudioMapperRuleReference): Array<string | number> | undefined {
...
export function mapperApplicableMetadata(rule: StudioMapperRuleReference | undefined, view: 'advanced' | 'all'): MapperAuthoringFieldMetadata[] {
  return mapperAuthoringMetadata.filter((field) => {
    const collection = field.path.startsWith('rules[]') ? 'rules' : field.path.startsWith('mappings[]') ? 'mappings' : undefined;
...
function normalizedActualPath(path: Array<string | number>): string {
  return path.reduce<string>((result, segment) => (typeof segment === 'number' ? `${result}[]` : result ? `${result}.${segment}` : segment), '');
}
...
function knownMapperPath(path: string): boolean {
  ...
  return (
    ...
    }) || ['mappings[].overlay.style', 'mappings[].conditions[].style'].some((prefix) => path === prefix || path.startsWith(`${prefix}.`))
  );
}
...
export function mapperStyleTarget(rule: unknown): StyleTargetKind | undefined {
  ...
  return kind && ['node', 'link', 'linkDirection', 'path', 'region'].includes(kind) ? (kind as StyleTargetKind) : undefined;
}

export function mapperStyleSlots(mapper: Record<string, unknown>, reference: StudioMapperRuleReference): StudioMapperStyleSlot[] {
  const collection = mapper[reference.collection];
  const rule = Array.isArray(collection) && isRecord(collection[reference.index]) ? collection[reference.index] : undefined;
  if (!rule) return [];
