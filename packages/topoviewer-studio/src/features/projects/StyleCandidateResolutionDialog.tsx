...
export function StyleCandidateResolutionDialog({
  candidate,
  context,
  onApply,
  onCancel,
  onDiscard,
  open
}: StyleCandidateResolutionDialogProps) {
    ...
    <StudioDialog maxWidth="xs" onClose={onCancel} open={open}>
      ...
      <StudioDialogContent>
        <Typography variant="body2">
          {context} would replace the current project while stylesheet changes are still unapplied.
          Apply the valid draft, or explicitly discard it before continuing.
        </Typography>
      ...
      <StudioDialogActions>
        ...
        <StudioButton
          ...
          disabled={state.status !== 'valid-dirty'}
          onClick={onApply}
        >Apply and continue</StudioButton>
      </StudioDialogActions>
    </StudioDialog>
  );
}
