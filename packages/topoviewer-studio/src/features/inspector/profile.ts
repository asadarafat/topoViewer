...
function preferenceFor(
  ...
  path: string
): StudioFieldPreference | undefined {
  return profile.fields.find((preference) => (
    preference.path === path && (!preference.targets?.length || preference.targets.includes(target))
  ));
}

export function resolveStudioFieldProfile(
  fields: readonly AuthoringFieldMetadata[],
  target: StyleTargetKind,
  profile: StudioAuthoringProfileOverride
): StudioResolvedFieldPreference[] {
...
export function updateStudioFieldPreference(
  profile: StudioAuthoringProfileOverride,
  target: StyleTargetKind,
  path: string,
  patch: Partial<Pick<StudioFieldPreference, 'hidden' | 'level' | 'order'>>
): StudioAuthoringProfileOverride {
  const existing = profile.fields.find((preference) => (
    preference.path === path && preference.targets?.length === 1 && preference.targets[0] === target
  ));
  const fields = profile.fields.filter((preference) => !(
    preference.path === path && preference.targets?.length === 1 && preference.targets[0] === target
  ));
  const next: StudioFieldPreference = { ...existing, path, targets: [target], ...patch };
...
export function reorderStudioFieldPreference(
  profile: StudioAuthoringProfileOverride,
  target: StyleTargetKind,
  fields: readonly AuthoringFieldMetadata[],
  path: string,
  ...
  const first = updateStudioFieldPreference(profile, target, current.path, { order: swap.order });
  return updateStudioFieldPreference(first, target, swap.path, { order: current.order });
}

export function migrateStudioAuthoringProfile(
  profile: StudioAuthoringProfileOverride,
  metadata: readonly AuthoringFieldMetadata[]
): StudioProfileMigrationResult {
  const removedPaths: string[] = [];
  const fields = profile.fields.filter((preference) => {
    const targets = preference.targets || [];
    const valid = metadata.some((field) => (
      field.path === preference.path
      && (!targets.length || targets.some((target) => field.targets.includes(target)))
    ));
    if (!valid) removedPaths.push(preference.path);
    return valid;
  });
  const warnings = profile.metadataVersion === studioAuthoringProfileVersion
    ? []
    : [`Migrated authoring profile metadata ${profile.metadataVersion} to ${studioAuthoringProfileVersion}.`];
