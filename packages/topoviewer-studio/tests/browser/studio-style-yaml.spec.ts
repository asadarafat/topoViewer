...
test('lazy loads embedded Monaco and exposes target-aware property, value, and selector completion', async ({ page }) => {
  ...
  await replaceCandidate(page, workspace, [
    'stylesheet:',
    '  - selector: node',
    '    style:',
    '      back'
...
test('keeps Style YAML search, rule navigation, source reveal, format warning, and diagnostics connected', async ({ page }) => {
  await page.goto('/?__studio-test-state=mapper-coverage');
  await page.locator('.react-flow__node[data-id="leaf1"]').click();
  const workspace = await openStudioWorkspace(page, 'Style');
  await workspace.getByRole('searchbox', { name: 'Search Basic style fields' }).fill('background color');
  ...
  await workspace.getByRole('button', { name: 'Open stylesheet source drawer' }).click();
...
test('contains an embedded editor failure while Basic and canvas remain usable', async ({ page }) => {
  ...
  await expect(workspace.getByRole('searchbox', { name: 'Search Basic style fields' })).toBeVisible();
  await expect(page.getByRole('region', { name: 'Topology canvas' })).toBeVisible();
