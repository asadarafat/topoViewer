...
async function sampledCandidateCommit(page: Page, footer: Locator, operation: (index: number) => Promise<number>) {
...
test('profiles visual expansion, search, selection, and candidate commits', async ({ page }) => {
  ...
  const candidateCommit = await sampledCandidateCommit(page, footer, async (index) =>
    timed(page, colorInput, 'keydown', async () => {
  ...
  for (const [name, series] of Object.entries(interactions)) {
    ...
    if (series.maximum >= budgets.budgets.browser.inspector.hardOutlierMs) {
      failures.push(`Basic style ${name} maximum ${series.maximum.toFixed(2)} ms exceeds ` + `${budgets.budgets.browser.inspector.hardOutlierMs} ms.`);
    }
  }
  try {
    expectBrowserSeriesWithinBudget(candidateCommit.settlement, budgets.budgets.browser.inspector.candidateSettleMedianMs, 'Basic style candidate settlement');
  } catch (error) {
    failures.push(error instanceof Error ? error.message : String(error));
  }
