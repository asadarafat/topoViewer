...
describe('Studio image export', () => {
  ...
  it('validates explicit bounded output dimensions', () => {
    expect(
      validateStudioImageExport({
        element: element(),
        options: { height: 720, kind: 'png', width: 1280 },
        snapshot
      })
    ...
    expect(() =>
      validateStudioImageExport({
        element: element(),
        options: { height: 9000, kind: 'png', width: 1280 },
        snapshot
      })
  ...
  it('rejects implicit remote asset fetches', () => {
    expect(() =>
      validateStudioImageExport({
        element: element({ remote: 'https://example.invalid/router.svg' }),
        options: { kind: 'svg' },
        snapshot
      })
