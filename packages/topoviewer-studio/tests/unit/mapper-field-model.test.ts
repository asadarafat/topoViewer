...
import { mapperApplicableMetadata, mapperFieldDisposition, mapperFieldIsDirectlyEditable, mapperFieldSourcePath, mapperStyleSlots, mapperStyleTarget, unknownMapperSourcePaths } from '../../src/features/mapper/mapperFieldModel';
...
describe('Studio mapper field model', () => {
  it('disposes every canonical metadata field as a control or reviewed YAML fallback', () => {
    ...
    expect(mapperFieldIsDirectlyEditable(mapperAuthoringField('rules[].metric')!, { collection: 'mappings', index: 0 })).toBe(false);
  });
  ...
  it('materializes selected rule paths and stops at unsupported nested sequences', () => {
    ...
    expect(mapperFieldSourcePath(mapperAuthoringField('mappings[].target.resolve.by')!, mapping)).toEqual(['mappings', 2, 'target', 'resolve', 'by']);
    expect(mapperFieldSourcePath(mapperAuthoringField('mappings[].conditions[].when.value.gte')!, mapping)).toEqual(['mappings', 2, 'conditions']);
  });
  ...
  it('derives target-compatible default and state style slots for both rule shapes', () => {
    ...
    expect(mapperStyleSlots(mapper, { collection: 'rules', index: 0 }).map((slot) => slot.label)).toEqual(['Default', 'busy']);
    expect(mapperStyleSlots(mapper, { collection: 'mappings', index: 0 }).map((slot) => slot.label)).toEqual(['Default', 'down']);
