...
describe('Studio visual node templates', () => {
  it('adds the trusted icon and node in one command plan when the stylesheet has no catalog', () => {
    ...
    expect(plan.value).toMatchObject({ icon: 'switch.generic', id: 'switch-1', name: 'New Switch' });
    expect(plan.additionalMutations).toEqual([expect.objectContaining({
      document: 'stylesheet',
      kind: 'upsert-value',
      path: ['icons', 'switch.generic'],
      scopePath: []
    })]);
  });

  it('preserves an existing project icon declaration', () => {
    const plan = createStudioPaletteNodePlan(document, {
      icons: { 'router.generic': { glyph: 'CUSTOM' } },
      stylesheet: []
    }, 'router', { x: 40, y: 80 });
    expect(plan.value.icon).toBe('router.generic');
