...
} from '../../src/features/inspector/profile';
...
describe('Studio authoring profiles', () => {
  it('layers sparse target-specific overrides over canonical metadata', () => {
    ...
    expect(promotedAndHidden.fields.find((field) => field.path === 'outlineWidth'))
      .toMatchObject({ hidden: true, level: 'basic' });
  });
  ...
  it('reorders fields without changing canonical metadata', () => {
    ...
    const resolved = resolveStudioFieldProfile(fields, 'node', reordered)
      .sort((left, right) => left.order - right.order || left.path.localeCompare(right.path));
    expect(resolved.findIndex((field) => field.path === 'width'))
      .toBeLessThan(resolved.findIndex((field) => field.path === 'shapePolygonPoints'));
    expect(resolved.findIndex((field) => field.path === 'width'))
      .toBeGreaterThan(resolved.findIndex((field) => field.path === 'nodeLayout'));
    expect(fields.find((field) => field.path === 'width')?.order).toBeGreaterThan(
      fields.find((field) => field.path === 'nodeLayout')?.order || -1
    );
  });
