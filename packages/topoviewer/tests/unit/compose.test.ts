...
import { composeTopoViewerDocument, type TopoDocument, type TopologyDocument } from '../../src';
...
describe('composeTopoViewerDocument', () => {
  ...
  it('rejects stylesheet-owned fields in topology input', () => {
    expect(() => composeTopoViewerDocument({
      graph: { id: 'invalid-ownership' },
      layout: { mode: 'manual' },
      stylesheet: [{ selector: 'node', style: { width: 80 } }]
    } as unknown as TopologyDocument)).toThrow(/layout is stylesheet-owned.*stylesheet is stylesheet-owned/);
  });
  ...
  it('falls back to stylesheet-owned fields when topology omits them', () => {
    ...
    expect(composed.layout).toEqual(stylesheet.layout);
    expect(composed.limits).toEqual(stylesheet.limits);
    expect(composed.toggles).toEqual([]);
    expect(composed.icons).toEqual(stylesheet.icons);
    expect(composed.labelFields).toEqual(stylesheet.labelFields);
    expect(composed.stylesheet).toEqual(stylesheet.stylesheet);
  });
