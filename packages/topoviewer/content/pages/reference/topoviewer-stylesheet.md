...
- Card nodes use `shape: roundRectangle` plus nested `nodeLayout.type: card`. `nodeLayout` changes the content layout inside the existing node body; it is not a separate shape.
...
- Regions: Defaults to `roundRectangle`; also default to `labelPosition: topLeft`, `labelMargin: 12`, `borderWidth: 1`, non-draggable, non-selectable, and `zIndex: -20`. Author both `width` and `height` for explicit geometry; otherwise auto-fit hulls use `padding`, `paddingX`, `paddingY`, `headerPadding`, `nodeWidth`, `nodeHeight`, `minWidth`, `minHeight`, `parentPadding`, `parentPaddingX`, and `parentPaddingY` from matching region rules.
...
Card layout is a node-internal layout, not a node shape. Keep
...
a circle, square, diamond, hexagon, or custom polygon.
