...
import type { DiagramCallout, DiagramShape, DiagramText, GraphEntity, IconSpec, StyleDeclaration, StylesheetDocument } from './types';
...
export function compileTextStyle(style: StyleDeclaration, entity: DiagramText) {
  const size = normalizeSize(entity.size, Number(style.width || styleDefaultNumber('text', 'width', 220)), Number(style.height || styleDefaultNumber('text', 'height', 64)));
