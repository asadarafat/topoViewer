...
See [Topology attention](../../attention.md) for the complete attention block reference, [Authoring for attention](create-a-layered-network-map.md#authoring-for-attention) for what to declare in topology YAML, and [Attention examples](../../reference/attention/index.md) for live examples with topology and stylesheet source.
...
- [Zensical](render-in-zensical.md): review the parallel static-docs adapter path.
- [Debug rendering](debug-rendering.md): compare MkDocs output with harness and Zensical when geometry differs.
