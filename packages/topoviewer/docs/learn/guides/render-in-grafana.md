...
Source loading is owned by the plugin backend. It scans the mounted bundle root,
rejects missing or duplicate canonical suffixes, parses the YAML, validates the
TopoViewer documents, and serves the selected bundle to the plugin frontend.

Telemetry loading is owned by Grafana. The query path calls Prometheus or
another data source and returns query responses as Grafana data frames. The
plugin frontend compiles the mapper YAML and resolves each telemetry sample
against the already-loaded TopoViewer inventory:
...
The renderer receives topology YAML, stylesheet YAML, and mapper-produced
runtime overlays in that order. Diagnostics are split by owner: backend source
diagnostics cover discovery, parsing, schema, and bundle-selection errors;
frontend runtime diagnostics cover mapper coverage, ambiguous matches,
unsupported overlays, render limits, and panel state.
