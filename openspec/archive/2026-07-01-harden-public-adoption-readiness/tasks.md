...
- [x] 25.4 Make README first screen show only install, render this YAML, and embed in React or MkDocs before secondary surfaces
- [x] 25.5 Demote Zensical, VS Code, Grafana, Containerlab, NetBox, and OpsMill/Infrahub from the README first-screen core while retaining status-labeled docs
...
- [x] 25.7 Create GitHub Release `v0.1.0` with release notes, changelog link, known limitations, install command, support status, and feedback issue link
- [x] 25.8 Add npm badge, package-size badge, live demo badge, and "works in 60 seconds" section after the package and demo URLs are real
- [x] 25.9 Decide whether `mkdocs-topoviewer` is ready for `pip install mkdocs-topoviewer`; do not advertise the command until publication is real
- [x] 25.10 Add a copyable demo gallery backlog with AWS VPC, Kubernetes service map, BGP/CLOS fabric, microservice dependency graph, incident blast-radius view, and Grafana live overlay
- [x] 25.11 Add at least one beautiful copyable gallery example beyond network-provider examples so non-network infrastructure users understand the value immediately
- [x] 25.12 Freeze and document the minimal public API target: `<TopoViewer document={document} />`, `compileTopoGraph(document)`, `validateTopoDocument(document)`, and `lintTopoDocument(document)`
- [x] 25.13 Replace or wrap weak public API types such as `Array<Record<string, unknown>>`, `ComponentType<any>`, and `unknown[]` before promoting a `1.0.0` stable-core promise
- [x] 25.14 Add typed public contracts for compiled graph data, extension hooks, events, node data, edge data, style declarations, and toolbar actions
- [x] 25.15 Decide Node compatibility for public package adoption: broaden runtime support to Node 20/22/24 where possible or document why Node 24 is required beyond repo tooling
...
- [x] 25.17 Track GitHub adoption baseline before launch: forks, releases, packages, external issues, and known first-user install issue
- [x] 25.18 Define the later `1.0.0` stable-core gate from real early-adopter feedback, API ownership review, compatibility fixtures, and migration policy
...
- [x] 26.3 Reshape docs home so the top flow is product identity, install, render your first topology, explore examples, and embed in React/MkDocs before telemetry, architecture, release, roadmap, or maintainer material
...
- [x] 26.6 Add expected screenshot or live viewport near the top of First Topology, plus "what you just built" and common blank-viewport mistake
...
- [x] 26.18 Add docs lint or public-readiness checks that fail if Start contains Tools, Evaluate, Labs, Maintainers, or roadmap-heavy pages
...
- [x] 26.21 Update curated example template so every curated example shows Live Viewport, Copy Topology YAML, Copy Stylesheet YAML, What this proves, and Use this when
- [x] 26.22 Review curated Examples visually so they feel like a gallery, not generated test catalog output
- [x] 26.23 Add visual review evidence for docs home, First Topology, Style Your First Topology, Examples Gallery, React, MkDocs, and Zensical adapter pages
...
- [x] 26.25 Update `spec-traceability.md`, `audit-traceability.md`, and the readiness report after the docs IA implementation is complete
...
- [x] 27.3 Push and verify GitHub CI and Docs workflows pass for the pushed branch
...
- [x] 27.6 Archive `openspec/changes/harden-public-adoption-readiness` only after local CI, remote CI/Docs, promo media decision, traceability, and readiness report are complete
