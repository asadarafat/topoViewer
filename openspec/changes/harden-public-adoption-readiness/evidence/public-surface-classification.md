...
| --- | --- | --- | --- |
| TopoViewer YAML model and renderer core | Supported | `packages/topoviewer`, `docs/topoviewer/reference-model.md`, generated schemas | Stable center of the repo. Public npm package still needs manual publish readiness before install docs can remove pre-publish caveat. |
| React embedding | Supported | `docs/topoviewer/react.md`, `packages/topoviewer/README.md` | Needs continued API/docs alignment and package install dry-run before public npm claim. |
| MkDocs plugin | Supported | `packages/mkdocs-topoviewer`, `docs/topoviewer/mkdocs.md`, docs CI | Supported docs embed path. |
| Zensical static adapter | Experimental | `docs/topoviewer/zensical.md`, generated Zensical site | Works as a generated adapter, but not a standalone packaged plugin. |
| Browser harness | Experimental | `packages/vscode-topoviewer/src/harness`, `docs/topoviewer/browser-harness.md` | Strong authoring surface, but mapper bundle authoring is still missing. |
...
| NetBox integration | Roadmap | `docs/topoviewer/integration-roadmap.md`, OpenSpec studies/changes | Not shipped. Should remain roadmap until plugin packaging and docs exist. |
| OpsMill/Infrahub integration | Roadmap | `docs/topoviewer/integration-roadmap.md`, OpenSpec studies/changes | Not shipped. Should remain roadmap until plugin packaging and docs exist. |
| Release, monorepo, docs standard, hardening docs | Maintainer | `docs/topoviewer/monorepo.md`, `docs/topoviewer/release.md`, `docs/topoviewer/docs-standard.md`, `docs/topoviewer/production.md` | Public for transparency, but not part of the first-run learning path. |
