...
Studio SHALL treat Basic/YAML synchronization, source preservation, contextual
...
- **WHEN** the browser and VS Code parity suites select node, link, region, and
  annotation targets and edit them through Basic and YAML
...
- **THEN** targeted Basic edits preserve every unrelated source range
...
#### Scenario: Change Basic controls repeatedly
...
Basic and YAML style modes SHALL meet the existing Studio WCAG 2.2 AA target.

#### Scenario: Author styles with a keyboard and screen reader

- **WHEN** an author navigates mode tabs, grouped fields, completion,
  diagnostics, Apply, Revert, source navigation, and inline migration
