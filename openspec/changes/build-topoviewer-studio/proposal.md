...
- Add canonical specification-driven style and mapper authoring metadata so the
  UI can expose the complete public contract through Basic, Advanced, All, and
  Modified views without duplicating runtime facts.
...
- Render every supported style attribute dynamically from canonical authoring
  metadata, with Basic, Advanced, All, and Modified views.
- Let users promote, demote, search, and reorder fields without changing the
